	...
	TBluetoothGattCharacteristic* const ACharacteristic, TBluetoothGattStatus AGattStatus)
{
	if(AGattStatus != TBluetoothGattStatus::Success) {
		...
		ManageCharacteristicData(ACharacteristic); //IT PROCESSES THE DATA AND UPDATES DE BPM DISPLAY
	}
}
//---------------------------------------------------------------------------
void __fastcall TfrmHeartMonitor::DoCharacteristicWrite(System::TObject* const Sender,
	TBluetoothGattCharacteristic* const ACharacteristic, TBluetoothGattStatus AGattStatus)
{
  if (AGattStatus != TBluetoothGattStatus::Success) {
	Memo1->Lines->Add("Error reading Characteristic " + ACharacteristic->UUIDName + ": " + IntToStr(static_cast<int>(AGattStatus)));
  }
  else
  {
	String LSValue = BytesToString(ACharacteristic->Value);
...
//---------------------------------------------------------------------------
String __fastcall TfrmHeartMonitor::BytesToString(const TBytes &B)
{
	String StrReturn = "";
	if(B.Length > 0) {
...
{
	if(AGattStatus != TBluetoothGattStatus::Success) {
		Memo1->Lines->Add("Error reading Characteristoc " + ADescriptor->UUIDName  + ": " +
...
void __fastcall TfrmHeartMonitor::EnableHRMMonitorize(bool Enabled)
{
	if(FHRMeasurementGattCharact != NULL) {
		...
		TBluetoothGattDescriptor* LDescriptor = FHRMeasurementGattCharact->Descriptors->Items[0];
		...
		if(Enabled) {
			btnMonitorize->Text = "Stop monitorize";
		} else {
			btnMonitorize->Text = "Start monitorize";
			ClearData();
		}
