...
uses
  ...
  FMX.ExtCtrls, FMX.Colors, FMX.Menus, FMX.TMSGridCell, FMX.TMSCustomGrid, FMX.StdCtrls;
...
begin
  ...
  TMSFMXGrid1.Cells[7, 2] := '100';
  TMSFMXGrid1.Cells[7, 5] := '100';
  TMSFMXGrid1.Cells[7, 7] := '100';
  TMSFMXGrid1.Cells[7, 10] := '100';
...
begin
  if CellEditor is TSwitch then
    (CellEditor as TSwitch).Tag := strtoint(TMSFMXGrid1.Cells[ACol, ARow]);
end;
...
begin
  case ACol of
    ...
    7:
    begin
      if (CellEditor as TSwitch).Tag > 0 then
        CellString := '100'
      else
        CellString := '0';
    end;
