...
uses
  ...
  FMX.Controls.Presentation, REST.Types;
...
type
  TSurfSpotFinder = class(TForm)
    ...
    procedure FixComa(Sender: TObject; AssignValueRec: TBindingAssignValueRec;
...
procedure TSurfSpotFinder.FixComa(Sender: TObject;
  AssignValueRec: TBindingAssignValueRec; var Value: TValue;
  var Handled: Boolean);
begin
  if not Value.IsEmpty then
    Value := StringReplace(Value.AsString, ',', '.', [rfIgnoreCase]);
end;
