...
begin
  ...
  // Создаем временный список для получения названий категорий
  CategoryList := TStringList.Create;
  try
    // Получаем все категории фильтров
    TFilterManager.FillCategory(CategoryList);
    // Заполняем список TListBox категориями
...
begin
  // Останавливаем анимацию для эффектов перехода
  ...
  // Создаем временный список для получения названий фильтров в категории
  FilterList := TStringList.Create;
  try
    // Получаем название фильтров в текущей категории
    TFilterManager.FillFiltersInCategory(CurrentCategory, FilterList);
    // Заполняем список TListBox названиями фильтров
    listFX.ItemIndex := -1;
...
begin
  ...
  // Удаляем старый аниматор
  RemoveTransitionAnimation;
  // Удаляем все контролы по управлению параметрами фильтра
  ClearFilterParamControls;
  // Удаляем старый фильтр
  RemoveFilter;
  // Создаем экземпляр нового фильтра
  CurrentFilterName := listFX.ListItems[listFX.ItemIndex].Text;
  FFilter := TFilterManager.FilterByName(CurrentFilterName);

  // Получаем информацию о параметрах фильтра и
  // строим панель с контролами по управлению параметрами фильтра
...
begin
  if FFilter <> nil then
  begin
    // Подаем на вход исходное изображение
    FFilter.ValuesAsBitmap['Input'] := Source.Bitmap;
    // Задаем второе изображение только для эффектов переходов.
    FFilter.ValuesAsBitmap['Target'] := Target.Bitmap;
    // Применяем и отображаем на форме в TImage
