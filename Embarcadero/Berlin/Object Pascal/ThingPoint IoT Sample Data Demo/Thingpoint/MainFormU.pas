...
uses CacheDataModuleU, EdgeServiceModuleU, FMX.DialogService.Sync;
...
begin
  ...
  HeartRateFrame.OnNextValue :=
    procedure (const AValue: TJSONObject)
    var
      LBeats: Integer;
    begin
      LBeats := 68 + Random(5);
      AValue.AddPair('bpm', TJSONNumber.Create(LBeats));
    end;
  ...
  EdgeServiceModule.OnModuleOverwrite :=
    ...
    begin
      Result := mrYes =
      TDialogServiceSync.MessageDialog(Format('ModuleName "%0:s" is already registered.  Overwrite "%0:s?"', [AModuleName]),
        TMsgDlgType.mtConfirmation, [TMsgDlgBtn.mbYes, TMsgDlgBtn.mbNo, TMsgDlgBtn.mbCancel], TMsgDlgBtn.mbYes, 0);
    end;
  ...
  EdgeServiceModule.OnProtocolPropsConflictDelete :=
    ...
    begin
      Result := mrYes =
      TDialogServiceSync.MessageDialog(Format('ModuleName "%0:s" has conflicting connection properties (%1:s).  Delete "%0:s?"', [AModuleName, AProtocolProps]),
        TMsgDlgType.mtConfirmation, [TMsgDlgBtn.mbYes, TMsgDlgBtn.mbNo, TMsgDlgBtn.mbCancel], TMsgDlgBtn.mbYes, 0);
    end;
