...
begin
  ...
  OSNameStr := Device.systemName.UTF8String;
  OSVersionStr :=  Device.systemVersion.UTF8String;
  DeviceTypeStr := Device.model.UTF8String;
  lbOSName.Text := Format('OS Name: %s', [OSNameStr]);
  lbOSVersion.Text := Format('OS Version: %s', [OSVersionStr]);
  lbDeviceType.Text := Format('Device Type: %s', [DeviceTypeStr]);
end;
{$ENDIF}
