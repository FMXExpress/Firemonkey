...
begin
  ...
  if OpenDialog1.Execute then
    ReadMaze;
  if MazeData.Points[1].Kind = TPathPointKind.CurveTo then
...
begin
  ...
  begin
    ...
    try
      ...
      begin
        if MazeData[i].Kind = TPathPointKind.MoveTo then
          ActionType := 'M'
        else
...
begin
  ...
  try
    ...
    begin
      if Path1.Data[i].Kind = TPathPointKind.MoveTo then
        ActionType := 'M'
      else
...
begin
  ...
  begin
    ...
    begin
      ...
      if MazeData.Points[i+1].Kind = TPathPointKind.MoveTo then
        continue;
