...
//---------------------------------------------------------------------------
TNeighborCount __fastcall TLifeEngine::TLifeThread::CountNeighbors(int Column, int Row)
{
	int _return = 0;
...
void __fastcall TLifeEngine::TLifeThread::ProcessCells(TObject * Sender, int AIndex)
{
  ...
  {
	  TNeighborCount NeighborCount = CountNeighbors(AIndex, i);
...
__fastcall TLifeEngine::TLifeEngine(const TSize ABoardSize)
{
	...
	FUpdateRate = 60;
	FLifeBoard.Length = ABoardSize.cx;
	for(int i = 0; i < FLifeBoard.Length; i ++) {
		FLifeBoard[i].Length = ABoardSize.cy;
    }
	Clear();
}
//---------------------------------------------------------------------------
...
int __fastcall TLifeEngine::GetNumber(const String ALine, int & Index)
{
	...
	return StrToIntDef(ALine.SubString(Start - 1, Index - Start), 0);
}
//---------------------------------------------------------------------------
	...
	TNeighborCounts &ASurvival, TNeighborCounts &ABirth)
{
	...
	while(LineNum < APattern->Count) {
		Line = APattern->operator [](LineNum);
...
void __fastcall TLifeEngine::LoadPattern(TStream *AStream)
{
	...
	while(ProcessPattern(Pattern.get(), Data.get(), Description.get(), Size, Offset, CurLine, FSurvival, FBirth)) {
		...
		Origin = Point(FLifeBoard.Length/2 + Offset.X, FLifeBoard[0].Length/2 + Offset.Y);
		for(int y = 0; y < Data->Count; y++) {
			Line = Data->operator [](y);
