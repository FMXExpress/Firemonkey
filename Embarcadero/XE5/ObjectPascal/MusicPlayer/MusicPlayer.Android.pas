...
uses
  ...
  Androidapi.JNI.Media, Androidapi.JNI.Provider, Androidapi.JNI.App;
type
...
//  FMusicPlayer.prepare;
  FPlayBackState := TMPPlaybackState.psStopped;
  FRepeatMode := TMPRepeatMode.rmAll;
  FShuffleMode := False;
...
begin
  FMusicPlayer.pause;
  FPlayBackState := TMPPlaybackState.psPaused;
end;

procedure TMusicPlayer.Play;
begin
//  if FPlayBackState = TMPPlaybackState.psStopped  then
//    FMusicPlayer.prepare;
  FMusicPlayer.start;
  FPlayBackState := TMPPlaybackState.psPlaying;
end;
...
begin
  if FPlayBackState = TMPPlaybackState.psPlaying then
...
begin
  ...
  begin
    case FMusicPlayer.PlaybackState of
      psPlaying: DoProcessPlay;
      psStopped,
      psPaused,
      psInterrupted,
      psSeekingForward,
      psSeekingBackward: sleep(200);
    end;
  end;
end;
