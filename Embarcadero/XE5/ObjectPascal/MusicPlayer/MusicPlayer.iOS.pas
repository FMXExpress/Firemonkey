...
type
  TMPControllerType = (ctApplication, ctIpod);
  TMPRepeatMode = (rmDefault, rmNone, rmOne, rmAll);
  TMPPlaybackState = (psStopped, psPlaying, psPaused, psInterrupted, psSeekingForward, psSeekingBackward);
  ...
  private
    ...
    FOnProcessPlay: TOnProcessPlayEvent;
    constructor Create(AType: TMPControllerType = ctApplication);
    destructor Destroy;
...
begin
  ...
  predicate :=TMPMediaPropertyPredicate.Wrap(TMPMediaPropertyPredicate.OCClass.predicateWithValue(TNSString.OCClass.stringWithString(NSSTR(AName)), MPMediaItemPropertyAlbumTitle));
...
begin
  ...
  begin
    case FMusicPlayer.PlaybackState of
      psPlaying: ProcessPlay;
      psStopped,
      psPaused,
      psInterrupted,
      psSeekingForward,
      psSeekingBackward: sleep(100);
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      FLastItem := FMusicPlayer.Playlist[FMusicPlayer.CurrentIndex];
      if Assigned(FMusicPlayer.OnSongChange) then
