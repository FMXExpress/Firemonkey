...
void __fastcall TMusicPlayer::DoOnSongChange(int newIndex) {
	if (FOnSongChange) {
		TThread::Queue(TThread::CurrentThread,
			new MyThreadMethod<TOnSongChangeEvent, int>(FOnSongChange, newIndex));
	}
}
//---------------------------------------------------------------------------
void __fastcall TMusicPlayer::DoOnProcessPlay(float newPos) {
	if (FOnProcessPlay) {
		TThread::Queue(TThread::CurrentThread,
			new MyThreadMethod<TOnProcessPlayEvent, float>(FOnProcessPlay, newPos));
	}
}
//---------------------------------------------------------------------------
System::DynamicArray<System::UnicodeString> __fastcall TMusicPlayer::GetAlbums(void) {
	...
	FAlbums[query->collections()->count().get()] = TMPAlbum::AllMusicAlbum();
	for (int i = 0; i < query->collections()->count().get(); i++) {
		...
		FAlbums[i].Name = TNSString::Wrap(item->representativeItem()->valueForProperty(MPMediaItemPropertyAlbumTitle()))->UTF8String();
		FAlbums[i].Artist = TNSString::Wrap(item->representativeItem()->valueForProperty(MPMediaItemPropertyArtist()))->UTF8String();
...
float __fastcall  TMusicPlayer::GetDuration(void) {
	return TNSNumber::Wrap(FMusicPlayer->nowPlayingItem()->valueForProperty(
...
//---------------------------------------------------------------------------
System::DynamicArray<System::UnicodeString> __fastcall TMusicPlayer::GetSongs(void) {
	System::DynamicArray<System::UnicodeString> _return;
	_di_MPMediaQuery query = TMPMediaQuery::Wrap(TMPMediaQuery::OCClass->songsQuery());
	...
	for (int i = 0; i < query->items()->count().get(); i++) {
		FPlaylist[i] = TMPSong::FromMediaItem(TMPMediaItem::Wrap(
			query->items()->objectAtIndex(NativeUInt(i))));
		_return[i] = Format("[%s]-[%s]", ARRAYOFCONST((FPlaylist[i].Artist, FPlaylist[i].Title)));
	}
	return _return;
}
//---------------------------------------------------------------------------
System::DynamicArray<System::UnicodeString> __fastcall TMusicPlayer::GetSongsInAlbum
	(System::UnicodeString AName)
{
	...
	System::DynamicArray<System::UnicodeString> _return;
	_di_MPMediaQuery query = TMPMediaQuery::Wrap(TMPMediaQuery::Alloc()->init());
	_di_MPMediaPropertyPredicate predicate = TMPMediaPropertyPredicate::Wrap(
		TMPMediaPropertyPredicate::OCClass->predicateWithValue(
			TNSString::OCClass->stringWithString(NSSTR(AName)), MPMediaItemPropertyAlbumTitle()
		)
	);
	...
	FPlaylist.set_length(query->items()->count().get());
	for (int i = 0; i < query->items()->count().get(); i++) {
		FPlaylist[i] = TMPSong::FromMediaItem(TMPMediaItem::Wrap(query->items()->objectAtIndex(NativeUInt(i))));
		_return[i] = Format("[%s]-[%s]", ARRAYOFCONST((FPlaylist[i].Artist, FPlaylist[i].Title)));
	}
	return _return;
}
...
NativeUInt __fastcall TMusicPlayer::IndexOfNowPlayingItem(void) {
	...
	for (int i = 0; i < FPlaylist.Length; i++) {
		if (FPlaylist[i].Equals(TMPSong::FromMediaItem(FMusicPlayer->nowPlayingItem()))) {
			_return = i;
			FCurrentIndex = i;
			break;
		}
...
void __fastcall TMusicPlayer::SetRepeatMode(const TMPRepeatMode Value) {
	FMusicPlayer->setRepeatMode(static_cast<NativeInt>(static_cast<int>(Value)));
}
//---------------------------------------------------------------------------
void __fastcall TMusicPlayer::SetShuffleMode(const bool Value) {
	if (Value) {
		FMusicPlayer->setShuffleMode(NativeInt(MPMusicShuffleModeSongs));
	} else {
...
__fastcall TMusicPlayer::TProcessThread::TProcessThread(bool CreateSuspended,
	TMusicPlayer* AMusicPlayer, Musicplayer::Utils::TOnProcessPlayEvent processHandler)
...
void __fastcall TMusicPlayer::TProcessThread::Execute(void) {
	while (FMusicPlayer) {
		switch(FMusicPlayer->PlaybackState) {
			case TMPPlaybackState::psPlaying :
				ProcessPlay();
				break;
			case TMPPlaybackState::psStopped :
			case TMPPlaybackState::psPaused :
			case TMPPlaybackState::psInterrupted :
			case TMPPlaybackState::psSeekingForward :
			case TMPPlaybackState::psSeekingBackward :
				Sleep(100);
				break;
        }
	}
...
void __fastcall TMusicPlayer::TProcessThread::ProcessPlay(void) {
	...
	if (FLastItem.Equals(TMPSong::EmptySong())) {
		...
		if (!FLastItem.Equals(FMusicPlayer->Playlist[FMusicPlayer->IndexOfNowPlayingItem().get()])) {
			FLastItem = FMusicPlayer->Playlist[FMusicPlayer->CurrentIndex];
			if (FMusicPlayer->OnSongChange) {
				FMusicPlayer->OnSongChange(FMusicPlayer->CurrentIndex);
			}
			Sleep(100);
		} else {
            Sleep(100);
