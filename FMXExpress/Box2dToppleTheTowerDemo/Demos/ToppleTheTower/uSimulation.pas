...
uses
  FMX.Types,
  FMX.Forms,
  UPhysics2DTypes, UPhysics2D, UPhysics2DHelper, uDebugDrawer, FMX.Objects;
...
type
  ...
  public
    ...
    procedure MouseUp(const p: TVector2; Launch: Boolean = True); virtual;
    procedure MouseMove(const p: TVector2);
    procedure LaunchBomb(velocity_factor: PhysicsFloat = 1.0; Obj: TFmxObject = nil); overload; virtual;
    procedure LaunchBomb(const position, velocity: TVector2; Obj: TFmxObject); overload;
  ...
  public
    ...
    procedure SetGUIForm(const Value: IGUIForm); virtual;
    procedure SetGUIBomb(const Value: TCircle); virtual;
...
begin
  ...
  body := BuildStaticBoundBox(140, 120, 0, 0, m_world);
...
procedure TSimulation.MouseUp(const p: TVector2; Launch: Boolean = True);
begin
   if Assigned(m_mouseJoint) then
   begin
   ...
   if Launch AND m_bombSpawning then
      CompleteBombSpawn(p);
end;
...
procedure TSimulation.LaunchBomb(velocity_factor: PhysicsFloat = 1.0; Obj: TFmxObject = nil);
var
   p, v: TVector2;
begin
   {$IFDEF OP_OVERLOAD}
...
procedure TSimulation.LaunchBomb(const position, velocity: TVector2; Obj: TFmxObject);
var
   bd: Tb2BodyDef;
...
begin
  if not m_bombSpawning then
   ...
   {$ENDIF}
   if Assigned(FGUIBomb) then
    begin
     LaunchBomb(m_bombSpawnPoint, vel, FGUIBomb);
    end
   else
     LaunchBomb(m_bombSpawnPoint, vel, nil);
   m_bombSpawning := False;
end;
...
procedure TSimulation.SetGUIBomb(const Value: TCircle);
begin
  FGUIBomb := Value;
end;
